...
private fun loadProperties(): Properties {
    ...
    try {
        ...
        FileInputStream(configFile).use { inputStream ->
            ...
            val knownKeys = listOf("OPENAI_API_KEY", "GEMINI_API_KEY", "ANTHROPIC_API_KEY", "IAEDU_GPT_API_KEY", "IAEDU_CLAUDE_API_KEY")
            val presentKeys = knownKeys.filter { properties.getProperty(it)?.isNotBlank() == true }
            if (presentKeys.isEmpty()) {
                ...
                println("✅ Found ${presentKeys.size} API key(s) in configuration: $presentKeys")
            }
        }
    } catch (e: Exception) {
