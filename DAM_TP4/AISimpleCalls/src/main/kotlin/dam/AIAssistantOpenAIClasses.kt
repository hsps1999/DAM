...
class AIAssistantOpenAIClasses(override val properties: Properties) : AIAssistant {
    ...
    override var model = "gpt-5.5" // ISEL - Available via institutional access
    ...
    override fun buildRequest(prompt: String): Request {
        ...
        val openAIRequest = OpenAIRequest(model = model, messages = messages, temperature = temperature, max_tokens = maxTokens)
    ...
    override fun makeApiCall(prompt: String): String {
        ...
        client.newCall(request).execute().use { response ->
            ...
            try {
                ...
                val truncated = if (responseBody.length > 200) "${responseBody.substring(0, 200)}..." else responseBody
                logger.error("Error parsing JSON response: ${e.message}")
                logger.error("Response body (truncated): $truncated")
